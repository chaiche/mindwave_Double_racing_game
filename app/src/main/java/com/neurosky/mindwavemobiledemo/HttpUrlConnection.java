...
public class HttpUrlConnection {
    ...
    //建構子(網路php路徑,要送的值)
    public HttpUrlConnection(String path,Map<String, String> map){

        this.map=map;
        Log.i("Http","success");
    }
    //發送HttpURLConnection_POST(網址,資料內容,編碼方式)
    public String sendHttpURLConnectionPOST(String path) {
        try {
    ...
    public String sendHttpURLConnectionGet(String path) {
        ...
        try {
            if (map != null && !map.isEmpty()) {            //判斷map是否非null或有初始化
                ...
                url = new URL(path+"?"+str);

                Log.d("url path",path+"?"+str);
                //將設置好的請求參數寫進dos
                //顯示時必須進行解碼,否則看到的中文會變成亂碼
                Log.i("text", "HttpURLConnection_POST.dos傳送資料=" + java.net.URLDecoder.decode(str, encode));
            }
        ...
        try {
            ...
            //打開服務器
            hucn =(HttpURLConnection) url.openConnection();
            hucn.setReadTimeout(5000);            //設置讀取超時為5秒
            hucn.setConnectTimeout(10000);        //設置連接網路超時為10秒
            //設置輸出入流串
            //hucn.setDoOutput(true);                //可寫入資料至伺服器
            ...
            e.printStackTrace();
            Log.e("text", "HttpURLConnection_Get=" + e.toString());
        }finally {
